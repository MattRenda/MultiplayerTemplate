...
namespace Mirror
{
    ...
    Styles styles = new Styles();
        ...
        // Ensure resources are properly released to avoid disposal warnings
        public override void Cleanup()
        {
            // Null any managed references
            title = null;
            styles = null;
            base.Cleanup();
        }

        // Extra safety: when Unity disables this preview, ensure Cleanup runs
        void OnDisable()
        {
            try { Cleanup(); } catch { }
        }

        ~NetworkInformationPreview()
        {
            // Final safeguard: ensure base cleanup is invoked even if Unity skips OnDisable
            try { Cleanup(); } catch { }
        }
